...
class SentimentClassificationModelBuid:
    def __init__(self, config):
        ...
        self.epochs_sentiment = config.epochs_sentiment
    ...
    def train_lstm_model(self, lstm_model, X_train, y_train, X_test, y_test, verbose=1):
        try:
            history = lstm_model.fit(X_train, y_train, 
                                batch_size=self.batch_size,
                                epochs=self.epochs_sentiment,
                                verbose=verbose)
    ...
    def run_pipeline(self, dataset_path):
        ...
        # Train LSTM model
        history = self.train_lstm_model(lstm_model, X_train, y_train, X_test, y_test)
...
model = SentimentClassificationModelBuid(config=Config)

# Run the entire pipeline
model.run_pipeline(dataset_path = Config.DATASET_SENTIMENT)
